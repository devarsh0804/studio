...
import { History, Loader2, PackagePlus, ScanLine, Search, ShoppingBag, Store, XCircle } from "lucide-react";
...
export function RetailerView() {
  ...
  const handleRetailerSubmit: SubmitHandler<RetailerFormValues> = (data) => {
    ...
    const newEvent: RetailEvent = { ...data, timestamp: new Date().toISOString() };
            ...
            <CardContent>
                <Form {...retailerForm}>
                    <form onSubmit={retailerForm.handleSubmit(handleRetailerSubmit)} className="space-y-4">
                        <FormField control={retailerForm.control} name="storeId" render={({field}) => (
                            <FormItem><FormLabel>Shelf / Store ID</FormLabel><FormControl><Input placeholder="STORE-567" {...field}/></FormControl><FormMessage/></FormItem>
                        )}/>
                        <FormField control={retailerForm.control} name="shelfDate" render={({field}) => (
                            <FormItem><FormLabel>Expiry / Shelf Date</FormLabel><FormControl><Input type="date" {...field}/></FormControl><FormMessage/></FormItem>
                        )}/>
